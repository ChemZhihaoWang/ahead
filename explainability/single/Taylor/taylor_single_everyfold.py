...
# Set global font to Arial and bold borders
plt.rcParams.update({
    'font.family': 'Arial',
    'axes.linewidth': 1.5,  # Bold borders
...
def set_tayloraxes(fig, location):
    ...
    CS = polar_ax.contour(ts, rs, rms, colors='gray', linestyles='--', linewidths=2)  # Bold dashed lines
    plt.clabel(CS, inline=1, fontsize=10)
    
    # Reference line for standard deviation of 1.0
    polar_ax.plot(np.linspace(0, np.pi/2, 100), np.ones(100), 'r--', linewidth=3, label="Std. Dev. = 1.0")  # Bold solid line
...
#----------------------------------------
# Specify colors for each model (example)
model_color_map = {
    'AdaBoost Regressor': '#E6194B',        # Red series
    'Bagging Regressor': '#3CB44B',         # Green
    'Bayesian Ridge Regression': '#0082C8', # Blue
    'CatBoost': '#F58231',                  # Orange
    'Decision Tree': '#911EB4',             # Purple
    'ElasticNet Regression': '#46F0F0',     # Cyan
    'Gradient Boosting': '#F032E6',         # Magenta
    'KNN Regression': '#D2F53C',            # Lime
    'Lasso Regression': '#FABEBE',          # Light pink
    'LightGBM': '#008080',                  # Teal
    'Linear Regression': '#E6BEFF',         # Lavender
    'Neural Network': '#AAFFC3',            # Light turquoise
    'Random Forest': '#808000',             # Olive
    'Ridge Regression': '#FFD8B1',          # Light apricot
    'Support Vector Machine': '#000080'     # Navy blue
}

#----------------------------------------

# Main directory path (please modify according to actual path)
main_folder_path = r'E:\desktop\hydro_channel\explainability\single'

# Get list of five-fold directories (can adjust according to actual situation)
fold_paths = [os.path.join(main_folder_path, f'fold_{i}') for i in range(1, 6)]

for fold_path in fold_paths:
    # Get all CSV files under the current fold
    ...
    for file_path in file_paths:
        model_name = os.path.splitext(os.path.basename(file_path))[0]  # Get model name from file name
        print(model_name)
        data = pd.read_csv(file_path)
        
        model_label = model_name.replace('_predictions_single', '')
    ...
    if len(model_names) > 0:
        ...
        # Save data to current fold directory
        ...
        for i, (index, row) in enumerate(metrics_df.iterrows()):
            # Get color from map, if not available use default color (e.g., black)
        ...
        # Save image to current fold directory
